...
export default function DeleteModal({ open, setOpen, color }) {
    ...
    const handleDelete = (color) => {
        ...
        colorService.delete(color).then((response) => {
            toastrService.message(response.data.message, "Başarılı", ToastrMessageType.success)
            getColors(colorService)
            setOpen(false)
        })
            .catch((error) => {
                toastrService.message(error.response.data.message, "Hata", ToastrMessageType.error)
                console.log(error)
            })
    }
                                        ...
                                        <div className="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            ...
                                            <div className="mt-2">
                                                <p className="text-sm text-gray-500">
                                                    Rengi silmek istediğinizden emin misiniz?
                                                </p>
